...
const CarDetails = () => {
  const { cars, axios, currency, pickupDate, returnDate, setPickupDate, setReturnDate } = useAppContext()
  const { id } = useParams()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const { data } = await axios.post('/api/bookings/create', { carId: id, pickupDate, returnDate })
      if (data.success) {
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
              {[
                ...
                <motion.div
                  ...
                  key={index + "carDetails"} className='flex flex-col items-center bg-light p-4 rounded-lg'>
